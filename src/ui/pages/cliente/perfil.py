...
def render_perfil(auth: dict) -> None:
    ...
    with ui.column().classes('w-full gap-6 p-6'):
        ...
        with ui.card().classes('w-full p-6 rounded-2xl gap-5'):
            ...
            def preencher_endereco_por_cep() -> None:
                try:
            ...
            buscar_cep.on('click', preencher_endereco_por_cep)
            cep.on('blur', lambda _: preencher_endereco_por_cep() if str(cep.value or '').strip() else None)
