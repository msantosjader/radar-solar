        ...
        with ui.column().classes('w-full max-w-7xl px-4 pt-8 gap-10'):
                ...
                with ui.column().classes('justify-center gap-6 py-4 rs-animate-up'):
                    ...
                    with ui.column().classes('w-full gap-4 pt-4 justify-between flex-1'):
                        with ui.row().classes('w-full gap-4 no-wrap items-stretch'):
                            for index, (eyebrow, value, label, icon, tone) in enumerate([
            ...
            with ui.column().classes('w-full items-center gap-4 pt-0 text-center rs-animate-up rs-animate-up-delay-2'):
                ...
                with ui.row().classes('w-full items-stretch justify-center gap-6 rs-grid'):
                    with ui.card().classes('rs-panel rs-card rs-animate-up rs-animate-up-delay-2 rounded-2xl p-8 gap-5 h-full items-center text-center'):
                        with ui.row().classes('items-center justify-center gap-4 w-full'):
                            ui.icon('bolt', size='34px').classes('text-secondary')
                            ui.label('Gerador de energia').classes('text-2xl font-bold text-slate-900')
                        ui.label('Acompanha a usina, entende a conta de luz e aciona suporte quando surge uma inconsistência.').classes(
                            'text-base text-slate-600 leading-7 text-center'
                        )
                        ...
                        ui.button('Seguir como gerador', on_click=lambda: ui.navigate.to('/login?profile=customer')).props(
                            'unelevated color=secondary').classes('mt-auto w-full py-3 font-semibold rounded-xl rs-button-soft')

                    with ui.card().classes('rs-panel rs-card rs-animate-up rs-animate-up-delay-3 rounded-2xl p-8 gap-5 h-full items-center text-center'):
                        with ui.row().classes('items-center justify-center gap-4 w-full'):
                            ui.icon('travel_explore', size='34px').classes('text-primary')
                            ui.label('Integrador solar').classes('text-2xl font-bold text-slate-900')
                        ui.label('Enxerga demanda real na região e transforma sinais do cliente em oportunidade comercial e técnica.').classes(
                            'text-base text-slate-600 leading-7 text-center'
                        )
                        with ui.column().classes('w-full items-center gap-3'):
                            for item in [
                                ...
                                ui.label(item).classes('max-w-md text-sm text-slate-700 text-center')
                        ui.button('Seguir como integrador', on_click=lambda: ui.navigate.to('/login?profile=company')).props(
                            'unelevated color=primary').classes('mt-auto w-full py-3 font-semibold rounded-xl rs-button-soft')
